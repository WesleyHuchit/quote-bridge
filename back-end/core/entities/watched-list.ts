export abstract class WatchedList<T> {
  ...
  public getModifiedItems(
    isModified: (initial: T, current: T) => boolean,
  ): T[] {
    return this.currentItems.filter((current) => {
      const initial = this.initial.find((i) => this.compareItems(i, current))
      return initial && isModified(initial, current)
    })
  }
}
